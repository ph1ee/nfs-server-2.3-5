...
define(AC_RPCGEN_C,
  ...
  AC_SUBST(RPCGEN_C)
  test -n "$RPCGEN_C" && AC_DEFINE(HAVE_RPCGEN_C, 1, "rpcgen -C")
])dnl
dnl
dnl *********** rpcgen -I ****************
define(AC_RPCGEN_I,
  ...
  AC_SUBST(RPCGEN_I)
  test -n "$RPCGEN_I" && AC_DEFINE(HAVE_RPCGEN_I, 1, "rpcgen -I")
])dnl
dnl
dnl *********** xdrproc_t ****************
define(AC_XDRPROC_T,
  ...
  test $nfsd_cv_type_xdrproc_t = yes && AC_DEFINE(HAVE_XDRPROC_T, 1, "xdrproc_t")
])dnl
dnl
dnl *********** xdr_free *****************
define(AC_XDR_FREE_DECL,
  [AC_MSG_CHECKING(for declaration of xdr_free)
  AC_CACHE_VAL(nfsd_cv_fn_xdr_free,
  [AC_EGREP_HEADER(xdr_free, rpc/xdr.h,
    nfsd_cv_fn_xdr_free=yes, nfsd_cv_fn_xdr_free=no)
  ]) dnl
  AC_MSG_RESULT($nfsd_cv_fn_xdr_free)
  test $nfsd_cv_fn_xdr_free = yes && AC_DEFINE(HAVE_XDR_FREE_DECL, 1, "xdr_free in header?")
])dnl
dnl
dnl *********** mountlist ****************
define(AC_MOUNTLIST,
  ...
  case $nfsd_cv_func_mountlist in
    fread_fstyp) AC_DEFINE(MOUNTED_FREAD_FSTYP, 1, "Mountlist: MOUNTED_FREAD_FSTYP");;
    getfsstat)	 AC_DEFINE(MOUNTED_GETFSSTAT, 1, "Mountlist: MOUNTED_GETFSSTAT");;
    getmnt)	 AC_DEFINE(MOUNTED_GETMNT, 1, "Mountlist: MOUNTED_GETMNT");;
    getmntent1)	 AC_DEFINE(MOUNTED_GETMNTENT1, 1, "Mountlist: MOUNTED_GETMNTENT1");;
    getmntent2)	 AC_DEFINE(MOUNTED_GETMNTENT2, 1, "Mountlist: MOUNTED_GETMNTENT2");;
    getmntinfo)	 AC_DEFINE(MOUNTED_GETMNTINFO, 1, "Mountlist: MOUNTED_GETMNTINFO");;
    vmount)	 AC_DEFINE(MOUNTED_VMOUNT, 1, "Mountlist: MOUNTED_VMOUNT");;
  esac
])dnl
dnl
dnl *********** FS usage *****************
define(AC_FSUSAGE,
  ...
  AC_CACHE_VAL(nfsd_cv_func_statfs,
    ...
    OSF1)		AC_DEFINE(STATFS_OSF1, 1, "FS usage: STATFS_OSF1");;
    read)		AC_DEFINE(STAT_READ, 1, "FS usage: STAT_READ");;
    statfs2_bsize)	AC_DEFINE(STAT_STATFS2_BSIZE, 1, "FS usage: STAT_STATFS2_BSIZE");;
    statfs2_fs_data)	AC_DEFINE(STAT_STATFS2_FS_DATA, 1, "FS usage: STAT_STATFS2_FS_DATA");;
    statfs2_fsize)	AC_DEFINE(STAT_STATFS2_FSIZE, 1, "FS usage: STAT_STATFS2_FSIZE");;
    statfs4)		AC_DEFINE(STAT_STATFS4, 1, "FS usage: STAT_STATFS4");;
    statvfs)		AC_DEFINE(STAT_STATVFS, 1, "FS usage: STAT_STATVFS");;
    esac
])dnl
dnl
dnl *********** libwrap bug **************
define(AC_LIBWRAP_BUG,
  [if test $ac_cv_lib_wrap_hosts_ctl = yes; then
    ...
    test $nfsd_cv_lib_wrap_bug = yes && AC_DEFINE(HAVE_LIBWRAP_BUG, 1, [Work around bug in
	  tcp_wrappers-7.2 where libwrap.a references deny_severity and allow_severity without
	  defining them])
  fi
])dnl
dnl
...
define(AC_DEV_T_SIZE,
  [AC_MSG_CHECKING(size of dev_t)
   ...
   [AC_TRY_RUN(
    ...
    AC_DEFINE(SIZEOF_DEV_T,$ac_cv_sizeof_dev_t, "sizeof(dev_t)")
  ])
dnl
dnl *********** sizeof(xxx_t) **************
...
define([AC_CHECK_SIZEOF],
  [changequote(<<, >>)dnl
   ...
   [AC_TRY_RUN(
    ...
    AC_DEFINE_UNQUOTED(AC_TYPE_NAME,$AC_CV_NAME, "sizeof")
...
define([AC_BSD_SIGNALS],
  ...
  AC_CACHE_VAL(nfsd_cv_bsd_signals,
    ...
    test $nfsd_cv_bsd_signals = yes && AC_DEFINE(HAVE_BSD_SIGNALS, 1, "BSD vs. POSIX signal handling")
])dnl
dnl
...
define([AC_AUTHDES_GETUCRED],
  [AC_MSG_CHECKING(for authdes_getucred in rpc/auth_des.h)
   ...
   test $nfsd_cv_getucred_declared = yes &&
    	AC_DEFINE(HAVE_AUTHDES_GETUCRED_DECL, 1, "authdes_getucred in header file?")
]) dnl
dnl
...
define([AC_BROKEN_SETFSUID],
  [if test $ac_cv_func_setfsuid = yes; then
	...
	AC_DEFINE(HAVE_BROKEN_SETFSUID, 1, "does setfsuid accept -2?")
   fi
]) dnl
