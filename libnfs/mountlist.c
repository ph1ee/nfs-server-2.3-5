	...
	int need_fs_type, all_fs;
{
...
#if defined (MOUNTED_GETFSSTAT)	/* __alpha running OSF_1 */
	{
		...
		for (counter = 0; counter < numsys; counter++) {
			me = (struct mount_entry *)
				xmalloc(sizeof(struct mount_entry));
			me->me_devname =
				xstrdup(stats[counter].f_mntfromname);
...
#ifdef MOUNTED_VMOUNT	/* AIX.  */
	{
		...
		mntctl(MCTL_QUERY, sizeof bufsize,
		       (struct vmount *) &bufsize);
		...
		for (thisent = entries; thisent < entries + bufsize;
			...
			if (vmp->vmt_flags & MNT_REMOTE) {
				...
				host = thisent +
					vmp->vmt_data[VMT_HOSTNAME].vmt_off;
				path = thisent +
					vmp->vmt_data[VMT_OBJECT].vmt_off;
				me->me_devname =
					xmalloc(strlen(host) + strlen(path) +
						2);
			...
			me->me_type =
				xstrdup(fstype_to_string(vmp->vmt_gfstype));
