...
static char log_name[NAME_MAX + 1];	       /* name of this program         */
...
log_open(char *progname, int foreground)
{
	...
	if (foreground) {
		...
		snprintf(path, PATH_MAX, LOG_FILE, progname);
	...
	snprintf(log_name, NAME_MAX, "%s[%d]", progname, (int) getpid());
}
...
log_toggle(int sig)
{
	dbg_printf(__FILE__, __LINE__, D_GENERAL, "turned off logging\n");
	logging = 1 - logging;
	dbg_printf(__FILE__, __LINE__, D_GENERAL, "turned on logging\n");
}

void
log_enable(char *kind)
{
	if ('a' == *kind && !strcmp(kind, "auth")) {
		...
		dbg_mask |= D_GENERAL;
	} else if ('m' == *kind && !strcmp(kind, "most")) {
		dbg_mask |= D_ALL & ~(D_UGID); 
...
void
dbg_printf(const char *file, int line, int kind, const char *fmt, ...)
{
	char buff[2048];
	va_list args;
	time_t now;
	...
	if (log_fp != (FILE *) NULL) {
		(void) time(&now);
		tm = localtime(&now);
		fprintf(log_fp, "%s %02d/%02d/%02d %02d:%02d %s %d : %s",
			log_name, tm->tm_mon + 1, tm->tm_mday, tm->tm_year,
			tm->tm_hour, tm->tm_min, file, line, buff);
		if (strchr(buff, '\n') == NULL)
			fputc('\n', log_fp);
	}
 ...
 */
void
log_call(const char* file, int line, struct svc_req *rqstp, char *xname, char *arg)
{
	char buffer[4096];
	size_t i, len, total;
	...
	total = sizeof(buffer);
	snprintf(buffer, total, "%s [%d ", xname, rqstp->rq_cred.oa_flavor);
	len = strlen(buffer);
