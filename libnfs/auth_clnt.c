...
seteids(uid_t cred_uid, gid_t cred_gid, gid_t * cred_gids, int cred_len)
{
	 ...
	 * Unless we're under Cygwin, in which case we don't need
	 * to be root, but we do need to be running under an account
	 * that has the proper security rights (ex, SYSTEM).
	 */
	...
	if (auth_gid != cred_gid) {
		...
		if (auth_uid != root_uid) {
			if (seteuid(root_uid) < 0)
				dbg_printf(__FILE__, __LINE__, L_ERROR,
					   "Unable to set effective uid for root: seteuid(%d): %s\n",
